...
const fs = require('fs');
var logger = require('log4js').getLogger("tabAnalysisReport");
...
router.post('/', function (req, res, next) {
    logger.info(`==================tabAnalysisReport页面====================`);
...
router.post('/simpleInputUpdate', function (req, res, next) {
    logger.info(`==================tabAnalysisReport页面更新代码安全缺陷类别至计划修复时间5个简单值====================`);
    logger.info(req.body);
...
router.post('/rtfHtml', function (req, res, next) {
    logger.info(`==================返回tabAnalysisReport页面富文本====================`);
    logger.info(req.body);
...
router.post('/rtfupload', function (req, res, next) {
    logger.info(`==================上传tabAnalysisReport页面富文本====================`);
    logger.info(req.body);
    let updateRow = req.body;

    logger.info(updateRow.system_name)
    logger.info(updateRow.system_name_id)
...
function resfileAbsPath(req, res, uploadDir) {
    ...
    form.parse(req, function (err, fields, files) {
        //logger.info('fields', fields);//表单传递的input数据  
        logger.info('files', files);//上传文件数据

        // froala-wysiwyg图片上传后需要返回{"link": "图片的绝对地址"}, 同时需要设置该目录资源为静态资源
        let absPath = files.file.path.substring(4).replace(/\\/g, '\/');
        logger.info(`http://localhost:3000/${absPath}`);
