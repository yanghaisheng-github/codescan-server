...
router.post('/srcupload/:language', upload.single('file'), function (req, res, next) {
  logger.info('---------------上传源码或者代码分析报告---------------------');
  let taskparas = [];
  //console.log(req);
  logger.info(`req.params.language: ${req.params.language}`);
...
router.post('/download', function (req, res, next) {
  ...
  /*   let filePath ='D:\\Projects\\Nodejs\\express-example\\' + req.body.url;
   logger.info("FileInfo: " + filePath);
    res.sendFile(filePath, function (err) {
      if (err) {
       logger.info("发送失败：" + err);
        next(err);
      } else {
       logger.info('发送成功:'+req.body.url);
      }
    }); */
  logger.info(`req.body.url: ${req.body.url}`);
  if (req.body.url) {
    ...
    logger.info(`发送的报文中没有文件对应路径，发送失败`);
...
router.post('/downloadReport', function (req, res, next) {
  logger.info('==============下载报告=====================');
  logger.info(req.body.systemname);
